  ...
  Versão: 1.0
 */
smartSig.registerCtrl("formImportarAluno", function($scope, $location, $http, $routeParams, Mensagem, $timeout, Permissao, $modal){
	...
	$scope.getLerPasta = function(){
		$http.get('api/index.php/lerpasta/alunos').    
	...
	$scope.importarAlunos = function() {  
		...
		console.log('importarAlunos', $scope.importar);
		for(var a=2; a <= $scope.importar.total; a=a+pct){
			...
			$http.post('api/index.php/importaraluno/', $scope.json, 
				{withCredentials: true,
					headers: {'enctype': 'multipart/form-data' },
				}
			).success(function(data, status, headers, config) {
				$scope.progressBar(data.importar_aluno.length);
