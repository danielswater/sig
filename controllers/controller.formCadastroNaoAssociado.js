smartSig.registerCtrl("formCadastroNaoAssociado", function($scope, $http, $routeParams, Mensagem, $timeout, $rootScope, $location, $modal, $filter, Permissao){
	...
	$scope.getCep = function(){
		...
		.success(function(data, status, headers, config) {
			...
			var cidades_estado = $filter('filter')(data[0].estados, {uf: $scope.endereco.estado});
	...
	*/
	...
	$scope.cadastrarPessoa = function() {
		if ($('#cadastroNaoAssociado-form').valid()) {
			SalvarDadosPessoais.disabled = true;
	...
	chkprc = false; // Conferir se está processando o POST (Uso abaixo em cadastrarTelefonePessoa)
	...
	$scope.cadastrarTelefonePessoa = function(objeto, pessoa) {		
		if ($('#cadastroTelefone-form').valid()) {
			if(!chkprc){
				...
				}).error(function(data, status) { chkprc = false; });			
			}
		}
	}
...
//@ sourceURL=controller.formCadastroNaoAssociado.js