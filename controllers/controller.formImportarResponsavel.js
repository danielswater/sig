  ...
  Versão: 1.0
 */
smartSig.registerCtrl("formImportarResponsavel", function($scope, $location, $http, $routeParams, Mensagem, $timeout, Permissao, $modal){
	...
	$scope.getLerPasta = function(){
		$http.get('api/index.php/lerpasta/responsavel').    
	...
	$scope.importarResponsavel = function() {  
		...
		for(var a=2; a <= $scope.importar.total; a=a+pct){
			...
			$http.post('api/index.php/importarresponsavel/', $scope.json, 
