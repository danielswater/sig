smartSig.registerCtrl("formCadastroAssociado", function($scope, $http, $routeParams, Mensagem, $timeout, $rootScope, $location, $modal, Permissao){
	...
	$scope.pessoas = [];
	$scope.pessoa = {};
	$scope.pessoa.id_tipo_documento=0;
	$scope.paises = {};
	$scope.estados = {};
	$scope.cidades = {};
	$scope.estados_civil = {};
	...
	$scope.getFonteCaptacao = function(){
		...
		success(function(data, status, headers, config) {
			if(data.error != -1){
				$scope.fontes_captacao = data.fonte_captacao;
			}
		}).
		error(function(data, status, headers, config) {
			// log error
	...
	$scope.getIndicador = function(){
		$http.get('api/index.php/consultapessoa/1').
		success(function(data, status, headers, config) {
			if(data.error != -1){
	...
	$scope.cadastrarPessoa = function() {
		if ($('#cadastroAssociado-form').valid()) {
			//SalvarDadosPessoais.disabled = true;
	...
	$scope.editarContribuicao = function(contrib){
		$scope.editContribuicao = true;
		$scope.addContrib = contrib;
	}
